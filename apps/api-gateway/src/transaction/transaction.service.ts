...
export class TransactionService implements OnModuleInit {
  ...
  async createTransaction(data: { amount: number; type: string; merchant: string; deviceId: string }) {
    ...
    const recentTx = await this.transactionRepo.createQueryBuilder('tx')
      .where('tx.deviceId = :deviceId', { deviceId: data.deviceId })
      .andWhere('tx.timestamp >= :tenMinsAgo', { tenMinsAgo })
      .getCount();
    
    if (recentTx >= 2) { // The current one will be the 3rd
      isAnomaly = true;
    ...
    const pastTx = await this.transactionRepo.createQueryBuilder('tx')
