...
describe('AccountController', () => {
  ...
  beforeEach(async () => {
    accountService = {
      ...
      listDevices: jest.fn().mockResolvedValue([{ deviceId: 'dev-1', username: 'testuser' }]),
    };
  ...
  describe('registerDevice', () => {
    it('registers a device with scoped username', async () => {
      ...
      expect(accountService.registerDevice).toHaveBeenCalledWith('dev-1', 'My Phone', 'secret-key', 'testuser');
    });
  });

  describe('getDevices', () => {
    it('fetches devices scoped to username', async () => {
      const result = await controller.getDevices({ user: { username: 'testuser' } });
