...
test.describe("App Frontend E2E", () => {
  test("should login and navigate to dashboard", async ({ page }) => {
    ...
    await expect(
      page.getByText("Connection to backend failed"),
    ).not.toBeVisible();
    ...
    await expect(
      page.getByText("Text-only mode optimized for slow networks"),
    ).toBeVisible();
    ...
    const grievanceInput = page
      .getByPlaceholder("Describe your issue...")
      .first();
    await grievanceInput.fill(
      "My money was deducted but not credited to merchant",
    );
    await page.getByText("Analyze Issue").click();

    // It should navigate to status page and show the intent
    await expect(page.getByText("transaction_failure")).toBeVisible({
      timeout: 15000,
    });
  });
});
